  ...
  return (
    <main className="min-h-screen bg-[#FAF9F6] text-slate-900">
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[1fr_320px] lg:px-8">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-black">{t('publicStore.storeInformation')}</h2>{profile.vacationMode && <div className="mt-4 rounded-xl bg-amber-50 p-4 text-sm text-amber-900">{profile.vacationMessage || t('publicStore.vacationDefault')}</div>}<p className="mt-4 text-sm text-slate-600">{t('publicStore.explore', { name: profile.businessName })}</p></div>
