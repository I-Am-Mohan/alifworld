...
export default function SellerApplicationPage() {
  ...
  const handleComplianceChange = (fieldId: string, val: string, field: any) => {
    if (fieldId in form) {
      setField(fieldId as keyof FormState, val);
    } else {
      setCustomComplianceValues((prev) => ({ ...prev, [fieldId]: val }));
    }

    const trimmed = val.trim();
    if (field.required && !trimmed) {
      setFieldError(fieldId, `${field.name} is a mandatory required field.`);
    } else if (trimmed && field.regex) {
      try {
