...
export default function ProductMediaManager({ productId }: { productId: string }) {
  ...
  const upload = async () => { if (!file) return; try { setSaving(true); setError(null); const form = new FormData(); form.set('file', file); form.set('mediaType', file.type.startsWith('video/') ? 'VIDEO' : 'IMAGE'); form.set('isPrimary', String(primary)); form.set('displayOrder', String(media.length)); const response = await csrfFetch(`/api/v1/seller/catalog/products/${productId}/media`, { method: 'POST', body: form }); const body = await response.json().catch(() => null); if (!response.ok || !body?.success) throw new Error(body?.error?.message || 'Unable to upload media.'); setFile(null); setPrimary(false); await load(); } catch (err: any) { setError(err.message || 'Unable to upload media.'); } finally { setSaving(false); } };
  const remove = async (item: Media) => { if (!window.confirm('Delete this media item?')) return; const response = await csrfFetch(`/api/v1/seller/catalog/products/${productId}/media/${item.id}`, { method: 'DELETE' }); const body = await response.json().catch(() => null); if (!response.ok || !body?.success) setError(body?.error?.message || 'Unable to delete media.'); else await load(); };
