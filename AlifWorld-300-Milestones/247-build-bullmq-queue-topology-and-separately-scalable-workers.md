...
milestone: 247
phase: 25
phase_name: "Notifications, Jobs, and Realtime Events"
title: "Build BullMQ queue topology and separately scalable workers"
status: not-started
currency: BDT
locales: [en-BD, bn-BD]
timezone: Asia/Dhaka
...
# Milestone 247: Build BullMQ queue topology and separately scalable workers

## Outcome

Implement **Build BullMQ queue topology and separately scalable workers** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 25, **Notifications, Jobs, and Realtime Events**. The phase purpose is: Move non-essential work to reliable queues and deliver localized multi-channel messages.
...
- Previous: [246 - Build in-app notification inbox, read state, and pagination](246-build-in-app-notification-inbox-read-state-and-pagination.md)
- Next: [248 - Implement recurring jobs and platform-timezone schedules](248-implement-recurring-jobs-and-platform-timezone-schedules.md)
...
- Workers share code and configuration with the one repository but run as separate processes.
- Ensure multiple workers can execute concurrently without duplicate side effects.
...
Primary expected areas for this phase: `features/notifications, workers, lib/queue, lib/outbox, app/api/v1/webhooks`. The exact existing repository structure wins when it already enforces the same boundaries. Do not create duplicate parallel modules merely to match a suggested path.
...
Phase-level test emphasis: deduplication, retry, dead letter, preference, outbox atomicity and provider-failure tests.
...
- [ ] The implementation of **Build BullMQ queue topology and separately scalable workers** is real, reachable where intended, and contains no fake success path.
