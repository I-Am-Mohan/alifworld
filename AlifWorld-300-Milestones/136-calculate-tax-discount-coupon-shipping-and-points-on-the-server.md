...
milestone: 136
phase: 14
phase_name: "Checkout and Shipping"
title: "Calculate tax, discount, coupon, shipping, and points on the server"
status: not-started
currency: BDT
locales: [en-BD, bn-BD]
timezone: Asia/Dhaka
...
# Milestone 136: Calculate tax, discount, coupon, shipping, and points on the server

## Outcome

Implement **Calculate tax, discount, coupon, shipping, and points on the server** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 14, **Checkout and Shipping**. The phase purpose is: Compute and commit a multi-seller checkout entirely on the server with Bangladesh-ready delivery.
...
- Previous: [135 - Build seller-level shipment groups for multi-vendor checkout](135-build-seller-level-shipment-groups-for-multi-vendor-checkout.md)
- Next: [137 - Implement COD eligibility, limits, and fraud-risk controls](137-implement-cod-eligibility-limits-and-fraud-risk-controls.md)
...
Primary expected areas for this phase: `features/checkout, features/shipping, services/checkout, app/api/v1/checkout`. The exact existing repository structure wins when it already enforces the same boundaries. Do not create duplicate parallel modules merely to match a suggested path.
...
Phase-level test emphasis: idempotency, totals, serviceability, COD risk, multi-seller and recovery tests.
...
- [ ] The implementation of **Calculate tax, discount, coupon, shipping, and points on the server** is real, reachable where intended, and contains no fake success path.
