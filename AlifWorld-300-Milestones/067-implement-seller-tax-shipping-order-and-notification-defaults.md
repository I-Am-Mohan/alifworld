...
milestone: 67
phase: 7
phase_name: "Seller Lifecycle"
title: "Implement seller tax, shipping, order, and notification defaults"
status: completed
...
# Milestone 067: Implement seller tax, shipping, order, and notification defaults

## Outcome

Implement **Implement seller tax, shipping, order, and notification defaults** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 07, **Seller Lifecycle**. The phase purpose is: Onboard, approve, operate, and govern sellers and their staff safely.
...
- Previous: [066 - Implement seller banking and payout profiles](066-implement-seller-banking-and-payout-profiles.md)
- Next: [068 - Implement seller suspension, restriction, and reactivation](068-implement-seller-suspension-restriction-and-reactivation.md)
...
- Treat tax rules as jurisdiction- and effective-date-aware configuration.
- Snapshot line and order tax breakdowns for invoices and refunds.
- Use explicit state transitions and append-only status history.
- Keep one customer parent view while sellers see only their fulfillment items.
- Respect channel preferences except mandatory security/transactional notices.
- Render templates by locale and deduplicate event delivery.
...
Primary expected areas for this phase: `features/sellers, app/seller, app/admin/sellers, app/api/v1/sellers`. The exact existing repository structure wins when it already enforces the same boundaries. Do not create duplicate parallel modules merely to match a suggested path.
...
Phase-level test emphasis: application state, KYC access, suspension, staff scope, storefront tests.
...
- [ ] The implementation of **Implement seller tax, shipping, order, and notification defaults** is real, reachable where intended, and contains no fake success path.
...
- **Files and migration:** Added migration `20260923090000_seller_operational_defaults`, seller operational-default and notification-default models, scoped repositories/services/routes, effective-date-aware tax fallback selection, OpenAPI documentation, architecture documentation, and focused tests.
- **Decisions:** This milestone persists seller defaults without changing checkout calculations, commission formulas, order transitions, payouts, or rewards. Tax jurisdiction/rule version/effective date are configuration metadata; no new tax rate is activated. Shipping mode and handling/cutoff values are defaults only. Security and transactional notifications remain mandatory regardless of seller preferences.
- **Behavior:** Seller owners can read/update operational and notification defaults through seller-scoped APIs. Seller staff are denied mutations. Defaults use optimistic concurrency and audit records. Notification channels/events are validated against the existing contract.
- **Validation:** `bun run db:generate` and `bun run typecheck` passed. Focused defaults/tax tests passed. `bun run openapi` and production build validation are required before commit.
- **Security and tenancy:** Every query and mutation requires seller scope; no client seller ID can cross the authenticated tenant. No financial or order-sensitive calculation was changed. Tax resolution now honors effective dates for existing rules.
- **Configuration:** No new environment variables are required. Existing PostgreSQL, JWT, notification, and seller authorization configuration remains required.
- **Exceptions and next milestone:** Checkout integration, seller-specific tax activation, shipping-rate changes, and auto-accept state transitions remain deferred pending approved business rules. Next: **068 — Implement seller suspension, restriction, and reactivation**.
