...
title: "Implement the required Bun command contract"
status: completed
currency: BDT
locales: [en-BD, bn-BD]
timezone: Asia/Dhaka
...
# Milestone 017: Implement the required Bun command contract

## Outcome

Implement **Implement the required Bun command contract** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 02, **Repository and Tooling**. The phase purpose is: Create a reproducible single Next.js codebase with strict quality gates and the exact Bun command contract.
...
- Previous: [016 - Implement typed environment validation and secret boundaries](016-implement-typed-environment-validation-and-secret-boundaries.md)
- Next: [018 - Establish unit, integration, and end-to-end test frameworks](018-establish-unit-integration-and-end-to-end-test-frameworks.md)
...
- Provide exactly `dev:local`, `dev:production`, `build:local`, and `production:production` scripts, plus the required database, OpenAPI, worker, lint, typecheck, and test scripts.
- `production:production` must run compiled output, never `next dev`.
...
- [x] The implementation of **Implement the required Bun command contract** is real, reachable where intended, and contains no fake success path.
