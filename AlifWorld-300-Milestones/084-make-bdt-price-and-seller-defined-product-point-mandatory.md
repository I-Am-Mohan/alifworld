...
milestone: 84
phase: 9
phase_name: "Products and Media"
title: "Make BDT price and seller-defined Product Point mandatory"
status: completed
...
# Milestone 084: Make BDT price and seller-defined Product Point mandatory

## Completion report (2026-09-24)

1. **Files and policies:** Made BDT currency and Product Point mandatory in product/variant validators and repository contracts; added service-level creation/update/publication enforcement; updated OpenAPI, focused tests, and architecture documentation.
2. **Decisions:** Product price is stored as integer poisha; Product Point is a separate seller-defined non-negative integer; no price-to-point conversion was introduced; historical order-item Product Point snapshots remain immutable.
3. **Behavior:** Product and variant requests reject missing or non-BDT currency, missing Product Points, negative Points, and invalid prices. Publication readiness repeats the checks server-side. Example BDT 10,000 is represented as 1,000,000 poisha independently from the seller-entered Product Point.
4. **Verification:** TypeScript typecheck, mandatory-field tests, existing catalog domain/product/draft tests, OpenAPI generation, and `git diff --check` passed.
5. **Security and integrity:** Seller-scoped draft and approval workflows remain enforced; Admin moderation cannot silently rewrite historical snapshots; no reward split, conversion, tax, commission, or payout behavior changed.
6. **Configuration:** No new credentials or migrations are required. Existing product, variant, order snapshot, and catalog migrations remain applicable.
7. **Approved exceptions and next milestone:** Existing legacy database rows are not rewritten by this validation milestone; future data-quality migration may backfill or quarantine invalid legacy records. Next milestone: 085, S3-compatible product image and video handling.

## Outcome

Implement **Make BDT price and seller-defined Product Point mandatory** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 09, **Products and Media**. The phase purpose is: Deliver production product and variant management with mandatory BDT price and Product Point values.
...
- Previous: [083 - Implement SKU, barcode, and uniqueness policies](083-implement-sku-barcode-and-uniqueness-policies.md)
- Next: [085 - Implement S3-compatible product image and video handling](085-implement-s3-compatible-product-image-and-video-handling.md)
...
- A seller enters both price and Product Point; publication fails if either is invalid.
- For the example phone, store BDT 10,000 as 1,000,000 poisha and Product Point as 1,000 independent units.
- Admin may moderate or cap Points through policy but may not silently alter historical snapshots.
- Keep Point units separate from BDT and from withdrawable balances.
- Snapshot Product Point on the order item before payment/fulfillment.
- Post and reverse through immutable events tied to source order items.
- Apply sellerId scope inside repository/service queries, not after data retrieval.
- Prevent Seller users from controlling approval, commission, protected payout, or another seller's objects.
- Validate ownership, status, category, variants, media, BDT price, Product Point, stock policy, and SEO before publication.
- Preserve public slug history or redirects when slugs change.
...
Primary expected areas for this phase: `features/products, app/seller/products, app/admin/products, app/api/v1/products`. The exact existing repository structure wins when it already enforces the same boundaries. Do not create duplicate parallel modules merely to match a suggested path.
...
Phase-level test emphasis: variant matrix, SKU uniqueness, point requirement, media validation, lifecycle tests.
...
- [ ] The implementation of **Make BDT price and seller-defined Product Point mandatory** is real, reachable where intended, and contains no fake success path.
