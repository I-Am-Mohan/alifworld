...
title: "Implement product weight, dimensions, tax, and shipping attributes"
status: completed
...
# Milestone 087: Implement product weight, dimensions, tax, and shipping attributes

## Completion report (2026-09-23)

1. **Files and migration:** Added product dimension, shipping class, and shipping-required fields with migration `20260923200000_product_shipping_attributes`; updated validators, ProductModel, repository persistence, OpenAPI, tests, and architecture documentation.
2. **Decisions:** Weight remains grams; dimensions are integer millimetres; shipping class is descriptive configuration; `requiresShipping` is explicit; no courier rate or fee is invented; existing effective-date tax rules and product/category overrides remain authoritative compatibility layers.
3. **Behavior:** Seller product create/update payloads now support validated physical dimensions and shipping attributes. Tax service continues effective-date-aware resolution, and order-item tax/Product Point snapshots remain historical.
4. **Verification:** Prisma validate/generate, TypeScript typecheck, shipping/tax/product/draft tests, OpenAPI generation, and `git diff --check` passed.
5. **Security and integrity:** Seller tenant scope and optimistic product updates are preserved; physical values reject negatives; no financial, tax-rate, commission, or reward formulas were invented.
6. **Configuration:** No new credentials are required. Deploy migration `20260923200000_product_shipping_attributes` before using persisted dimensions/shipping fields.
7. **Approved exceptions and next milestone:** Courier-specific rate calculation, volumetric-weight policy, and shipping-label generation remain future logistics work. Next milestone: 088, product submission, approval, publication, and archival.

## Outcome

Implement **Implement product weight, dimensions, tax, and shipping attributes** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 09, **Products and Media**. The phase purpose is: Deliver production product and variant management with mandatory BDT price and Product Point values.
...
- Previous: [086 - Implement localized descriptions, specifications, and rich content](086-implement-localized-descriptions-specifications-and-rich-content.md)
- Next: [088 - Implement product submission, approval, publication, and archival](088-implement-product-submission-approval-publication-and-archival.md)
...
- [ ] The implementation of **Implement product weight, dimensions, tax, and shipping attributes** is real, reachable where intended, and contains no fake success path.
