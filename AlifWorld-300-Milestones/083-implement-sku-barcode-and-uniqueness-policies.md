...
title: "Implement SKU, barcode, and uniqueness policies"
status: completed
...
# Milestone 083: Implement SKU, barcode, and uniqueness policies

## Completion report (2026-09-24)

1. **Files and APIs:** Added normalized SKU/barcode contracts, identifier policy service, seller identifier check API, ProductService enforcement, OpenAPI documentation, focused tests, and architecture documentation.
2. **Decisions:** SKUs normalize to uppercase; barcodes normalize whitespace and accept 8–14 digits; EAN-13 checksum validation is available; products and variants share one service-level collision domain.
3. **Behavior:** Product creation and updates reject conflicting identifiers across products and variants. Seller clients can check identifier availability before saving drafts. Optimistic draft updates and existing database SKU constraints remain intact.
4. **Verification:** TypeScript typecheck, identifier-policy tests, variant-combination tests, draft-editing tests, OpenAPI generation, and `git diff --check` passed.
5. **Security and integrity:** Identifier checks run server-side; product and variant ownership boundaries remain unchanged; no pricing, Product Point, tax, or reward behavior changed.
6. **Configuration:** No new credentials or migrations are required. Existing product and variant identifier constraints remain authoritative.
7. **Approved exceptions and next milestone:** Cross-table database-level barcode uniqueness is deferred because products and variants use separate tables; service-level checks are enforced now. Next milestone: 084, mandatory BDT price and seller-defined Product Point.

## Outcome

Implement **Implement SKU, barcode, and uniqueness policies** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 09, **Products and Media**. The phase purpose is: Deliver production product and variant management with mandatory BDT price and Product Point values.
...
- Previous: [082 - Implement variant combination generation and validation](082-implement-variant-combination-generation-and-validation.md)
- Next: [084 - Make BDT price and seller-defined Product Point mandatory](084-make-bdt-price-and-seller-defined-product-point-mandatory.md)
...
- [ ] The implementation of **Implement SKU, barcode, and uniqueness policies** is real, reachable where intended, and contains no fake success path.
