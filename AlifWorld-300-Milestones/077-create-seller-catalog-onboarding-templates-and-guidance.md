...
title: "Create seller catalog onboarding templates and guidance"
status: completed
...
# Milestone 077: Create seller catalog onboarding templates and guidance

## Completion report (2026-09-23)

1. **Files and migration:** Added `CatalogOnboardingTemplate` and `SellerCatalogOnboardingProgress` with migration `20260923160000_seller_catalog_onboarding`; onboarding validators, repository, service, APIs, Seller guidance UI, OpenAPI schemas, focused tests, and architecture documentation.
2. **Decisions:** Templates are Admin-governed, category-scoped or global, localized by `bn-BD`/`en-BD`, and informational only. Seller progress is scoped by authenticated seller ownership/staff access and stored through an idempotent upsert.
3. **Behavior:** Added category template lookup, Admin template CRUD, seller progress listing and updates, required-field completion percentage, media/title/description guidance, empty/error/retry UI states, and a Seller Catalog navigation link.
4. **Verification:** Prisma validate/generate, TypeScript typecheck, focused onboarding/approval/attribute tests, OpenAPI generation, and `git diff --check` passed. Production build remains to be run in the final validation pass.
5. **Security and integrity:** Seller scope is enforced inside the service and repository queries; arbitrary seller IDs are not accepted by normal Seller progress routes; Admin mutations are protected and audited; guidance does not grant catalog approval or publication permissions.
6. **Configuration:** No new credentials are required. Deploy migration `20260923160000_seller_catalog_onboarding` before enabling persisted templates and progress.
7. **Approved exceptions and next milestone:** Initial template seed content is intentionally not inserted automatically because category-specific business guidance requires approved content. Next milestone: 078, safe bulk catalog import and export.

## Outcome

Implement **Create seller catalog onboarding templates and guidance** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 08, **Catalog Foundations**. The phase purpose is: Build governed taxonomy and catalog primitives usable by sellers, customers, Admin, and search.
...
- Previous: [076 - Implement catalog permissions and product approval workflow](076-implement-catalog-permissions-and-product-approval-workflow.md)
- Next: [078 - Implement safe bulk catalog import and export](078-implement-safe-bulk-catalog-import-and-export.md)
...
- [ ] The implementation of **Create seller catalog onboarding templates and guidance** is real, reachable where intended, and contains no fake success path.
