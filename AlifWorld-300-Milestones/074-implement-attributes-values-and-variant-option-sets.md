...
title: "Implement attributes, values, and variant option sets"
status: completed
...
# Milestone 074: Implement attributes, values, and variant option sets

## Completion report (2026-09-23)

1. **Files and migration:** Added normalized Prisma models and migration `20260923130000_catalog_attributes`; attribute/value/category-assignment validators, repositories, services, routes, types, IDs, OpenAPI schemas, focused tests, and architecture documentation.
2. **Decisions:** Attributes and values are Admin-governed; category assignments define required and variant-defining behavior; seller owners/staff manage only their own product option sets and variant options; legacy flattened variant fields remain for compatibility.
3. **Behavior:** Added Admin CRUD for attributes and values, category assignment replacement, public active attribute reads, seller option-set replacement, normalized variant-option replacement, value-to-attribute validation, duplicate-attribute rejection, optimistic version checks, and audit records for Admin mutations.
4. **Verification:** `bunx prisma validate`, `bunx prisma generate`, focused catalog tests, `bun test tests/unit/catalog-attributes.test.ts`, `bun run typecheck`, `bun run openapi`, and `git diff --check` passed. Full production build was not repeated after this milestone-only change; the previous milestone build was green.
5. **Security and integrity:** Admin authorization, seller tenant checks, active/deleted filtering, governed value ownership checks, duplicate prevention, and optimistic concurrency are implemented. No financial, tax, Product Point, or reward behavior changed.
6. **Configuration:** No new credentials are required. Deploy migration `20260923130000_catalog_attributes` before enabling the new routes.
7. **Approved exceptions and next milestone:** Full variant-combination completeness and publication-readiness enforcement are deferred to the product approval workflow. Next milestone: 075, taxonomy translation and SEO controls.

## Outcome

Implement **Implement attributes, values, and variant option sets** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 08, **Catalog Foundations**. The phase purpose is: Build governed taxonomy and catalog primitives usable by sellers, customers, Admin, and search.
...
- Previous: [073 - Implement curated and rule-based collections](073-implement-curated-and-rule-based-collections.md)
- Next: [075 - Add taxonomy translation and SEO controls](075-add-taxonomy-translation-and-seo-controls.md)
...
- [ ] The implementation of **Implement attributes, values, and variant option sets** is real, reachable where intended, and contains no fake success path.
