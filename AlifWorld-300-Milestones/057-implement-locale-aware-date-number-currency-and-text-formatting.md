...
title: "Implement locale-aware date, number, currency, and text formatting"
status: completed
...
# Milestone 057: Implement locale-aware date, number, currency, and text formatting

## Outcome

Implement **Implement locale-aware date, number, currency, and text formatting** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 06, **Bangladesh Localization**. The phase purpose is: Make Bangladesh the first operational market while keeping locale, currency, and geography extensible.
...
- Previous: [056 - Model Bangladesh phone, address, and geographic hierarchy data](056-model-bangladesh-phone-address-and-geographic-hierarchy-data.md)
- Next: [058 - Localize validation, errors, notifications, and transactional content](058-localize-validation-errors-notifications-and-transactional-content.md)
...
- [ ] The implementation of **Implement locale-aware date, number, currency, and text formatting** is real, reachable where intended, and contains no fake success path.
...
1. **Files changed**
   - Made `src/shared/utils/localization.ts` the shared formatting façade for locale-aware numbers, exact BDT/poisha currency, Dhaka date/time output, and safe text interpolation.
   - Updated currency/i18n helpers and root layout locale hydration in `src/shared/utils/currency.ts`, `src/i18n/context.tsx`, `src/i18n/server.ts`, and `src/app/layout.tsx`.
   - Replaced unsafe UI `Number(poisha)` conversions and browser-local date/time formatting across customer, admin, order, wallet, and seller surfaces.
   - Added `tests/unit/locale-formatting.test.ts`.

2. **Decisions and source authority**
   - Preserved the existing `en-BD`/`bn-BD` locale contract, default `bn-BD`, BDT minor-unit invariant, Bengali numeral behavior, and `Asia/Dhaka` timezone from prior accepted localization and domain decisions.
   - Kept monetary values as bigint poisha through formatting; no exchange rates, taxes, financial rules, or Point conversions were introduced.
   - Kept text interpolation literal and placeholder-based; no markup evaluation or new translation framework was added.

3. **Behavior added**
   - Locale-aware number formatting with Bengali numeral output for `bn-BD` and ASCII output for `en-BD`.
   - Exact BDT currency formatting for zero, negative, fractional, and values beyond JavaScript safe integer range.
   - Date-only, time-only, and datetime formatting pinned to `Asia/Dhaka`.
   - Invalid-date and non-finite-number rejection.
   - Server and client translation interpolation now use the shared safe text formatter.
   - Root layout passes the server-negotiated locale to `I18nProvider` and the HTML `lang` attribute.
   - Customer, admin, and seller financial/order pages use shared formatting instead of local monetary/date implementations.

4. **Tests and commands run**
   - `bun test tests/unit/locale-formatting.test.ts tests/unit/currency-and-points.test.ts tests/unit/date-and-timezone.test.ts tests/unit/locale-resolution-and-routing.test.ts` — passed, 47 tests.
   - `bun run typecheck` — passed.
   - `bun run openapi` — passed; regenerated the authoritative specification.
   - `bun run build:local` — passed; existing unrelated React Hook/image lint warnings remain.
   - `git diff --check` — passed.

5. **Security, tenancy, localization, and financial-integrity checks**
   - Formatting is presentation-only and does not mutate financial values or convert bigint monetary data through `number`.
   - User interpolation values are treated as literal text and are not evaluated as markup.
   - Locale normalization continues to accept only the established launch locales and fallback behavior.
   - UI date output explicitly uses `Asia/Dhaka`; server/client locale initialization is aligned to reduce hydration mismatch.
   - No new protected resource, tenancy boundary, credential, worker, cache, or provider behavior was introduced.
...
7. **Exceptions, unresolved gates, and next milestone**
   - Existing unrelated build warnings remain in admin setup, authentication context, and image usage; they do not block this milestone.
   - Approved financial, legal, tax, consumer-protection, reward, withdrawal, promotional, and advertising gates remain unresolved and were intentionally not changed.
   - Exact next milestone: **058 — Localize validation, errors, notifications, and transactional content**.
