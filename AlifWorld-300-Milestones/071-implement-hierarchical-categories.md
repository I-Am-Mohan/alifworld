...
title: "Implement hierarchical categories"
status: completed
...
# Milestone 071: Implement hierarchical categories

## Outcome

Implement **Implement hierarchical categories** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 08, **Catalog Foundations**. The phase purpose is: Build governed taxonomy and catalog primitives usable by sellers, customers, Admin, and search.
...
- Previous: [070 - Complete seller lifecycle audits and acceptance tests](070-complete-seller-lifecycle-audits-and-acceptance-tests.md)
- Next: [072 - Implement brands and brand approval](072-implement-brands-and-brand-approval.md)
...
- [ ] The implementation of **Implement hierarchical categories** is real, reachable where intended, and contains no fake success path.
...
- **Files changed:** Hardened category hierarchy service/repository behavior, added cycle-safe ancestor validation and full recursive tree assembly, public and Admin category APIs, functional Admin category tree/create UI, OpenAPI schemas/routes, architecture documentation, and focused tests. No Prisma migration was required because the existing self-referencing Category model and indexes support this scope.
- **Decisions:** Active category hierarchy is returned recursively from top-level nodes. Parent changes are rejected when they would create self/descendant cycles. Category writes require Admin catalog authorization and optimistic versions; public reads expose active hierarchy only.
- **Behavior:** `GET /api/v1/categories` returns the public active tree. Admins can `GET/POST /api/v1/admin/categories` and `PATCH /api/v1/admin/categories/{id}`. Slugs are normalized/validated and duplicate slugs conflict. Category tax fields remain configuration metadata; no new tax or financial behavior is activated.
- **Validation:** `bun run typecheck`, focused category tests, `bun run openapi`, `bun run build:local`, and `git diff --check` passed. Existing repository lint warnings remain.
- **Security and tenancy:** Admin category mutations require catalog write authorization; public reads contain active categories only; parent lookups and cycle traversal are database-backed. No seller can modify taxonomy through these routes, and no financial behavior changed.
- **Configuration:** No new environment variables or migrations are required.
- **Exceptions and next milestone:** Category translation editing, image upload lifecycle, moderation workflow, and deep category-specific tax-rule activation remain later catalog milestones. Next: **072 — Implement brands and brand approval**.
