...
title: "Build seller staff administration and activity visibility"
status: completed
...
# Milestone 069: Build seller staff administration and activity visibility

## Outcome

Implement **Build seller staff administration and activity visibility** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 07, **Seller Lifecycle**. The phase purpose is: Onboard, approve, operate, and govern sellers and their staff safely.
...
- Previous: [068 - Implement seller suspension, restriction, and reactivation](068-implement-seller-suspension-restriction-and-reactivation.md)
- Next: [070 - Complete seller lifecycle audits and acceptance tests](070-complete-seller-lifecycle-audits-and-acceptance-tests.md)
...
- [ ] The implementation of **Build seller staff administration and activity visibility** is real, reachable where intended, and contains no fake success path.
...
- **Files changed:** Added seller staff activity query/API, strengthened staff UI to use real persisted APIs, added safe staff activity documentation, canonical staff-added audit action, OpenAPI documentation, and focused regression coverage. No Prisma migration was required because existing `SellerStaff` and `AuditLog` models were sufficient.
- **Decisions:** Seller owners manage staff; seller staff cannot administer staff by default. All staff reads/activity queries are seller-scoped in the database. Activity visibility is limited to staff-resource audit entries for the active seller tenant and does not expose unrelated platform audit data.
- **Behavior:** `GET/POST/DELETE /api/v1/seller/staff` remains the staff management contract; `POST /api/v1/seller/staff/invite` remains supported; `GET /api/v1/seller/staff/activity` provides bounded paginated activity. The UI now loads actual staff records, sends real invitations, and shows loading, error, empty, and activity states instead of fabricated local records.
- **Validation:** `bun run typecheck` passed. Focused seller tenancy/lifecycle tests passed: 10 tests, 18 assertions. OpenAPI generation, production build, and whitespace validation are required before commit.
- **Security and tenancy:** Cross-tenant requests fail closed; owner cannot be removed as staff; seller staff cannot manage staff; audit activity is filtered by seller ID and returns only safe fields. No financial, payout, commission, reward, or approval behavior changed.
- **Configuration:** No new environment variables or migrations are required. Existing authentication, seller authorization, Prisma, and audit configuration remains required.
- **Exceptions and next milestone:** Invitation delivery remains represented by the existing outbox/audit boundary; external email/SMS delivery is not invented here. Next: **070 — Complete seller lifecycle audits and acceptance tests**.
