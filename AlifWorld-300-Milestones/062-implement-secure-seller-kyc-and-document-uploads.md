...
title: "Implement secure seller KYC and document uploads"
status: completed
...
# Milestone 062: Implement secure seller KYC and document uploads

## Outcome

Implement **Implement secure seller KYC and document uploads** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 07, **Seller Lifecycle**. The phase purpose is: Onboard, approve, operate, and govern sellers and their staff safely.
...
- Previous: [061 - Build the seller application workflow](061-build-the-seller-application-workflow.md)
- Next: [063 - Build Admin review, approval, rejection, and resubmission flows](063-build-admin-review-approval-rejection-and-resubmission-flows.md)
...
- Require measurable completion signals and anti-bot/device abuse controls.
- Do not describe projected earnings as guaranteed.
- Keep campaign billing, user rewards, and affiliate commission as separate ledgers.
- Encrypt or strongly protect document access, use short-lived signed URLs, and log every view.
- Define retention and reviewer permissions before collection.
...
- [ ] The implementation of **Implement secure seller KYC and document uploads** is real, reachable where intended, and contains no fake success path.
...
- **Files and migration:** Added the S3-compatible private object storage adapter, KYC integrity metadata migration `20260923050000_secure_seller_kyc_document_metadata`, secure multipart upload validation, signed view URL service, seller and Admin KYC routes, functional seller KYC UI, OpenAPI paths, architecture documentation, translations, and regression tests.
- **Decisions:** KYC files are server-uploaded to private S3-compatible storage and represented in the database by private object keys. Supported files are PDF, JPEG, PNG, and WebP up to 10 MB. SHA-256 duplicate detection, MIME/magic-byte validation, authenticated device identification, and five uploads per user/device/IP window provide measurable abuse controls.
- **Behavior:** Seller owners can upload and list documents, and can request 15-minute signed view URLs. Admins with `sellers:verify` can list pending documents and verify or reject them with optimistic version checks. Every document view and review is audited; signed URL contents are never logged.
- **Validation:** `bun run db:generate`, `bunx prisma validate`, `bun run typecheck`, focused KYC and tenant-isolation tests, `bun run openapi`, and `bun run build:local` passed. Existing full-suite baseline failures remain unrelated to this milestone.
- **Security and tenancy:** Upload authorization uses authenticated seller ownership/staff checks and route policy checks. Seller document reads are re-queried with seller scope. Public URLs are not returned, credentials are not exposed, and no document content is logged. Financial, commission, payout, and reward behavior was unchanged.
- **Configuration:** Existing S3-compatible environment variables are required. Production must use non-default credentials, private bucket policy, and the approved retention/malware scanning configuration.
- **Exceptions and next milestone:** Retention duration, legal hold, malware scanning provider, and permanent deletion policy remain approval gates. Next: **063 — Build Admin review, approval, rejection, and resubmission flows**.
