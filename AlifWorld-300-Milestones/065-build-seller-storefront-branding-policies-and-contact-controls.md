...
title: "Build seller storefront branding, policies, and contact controls"
status: completed
...
# Milestone 065: Build seller storefront branding, policies, and contact controls

## Outcome

Implement **Build seller storefront branding, policies, and contact controls** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 07, **Seller Lifecycle**. The phase purpose is: Onboard, approve, operate, and govern sellers and their staff safely.
...
- Previous: [064 - Create seller accounts, profiles, and public store identities](064-create-seller-accounts-profiles-and-public-store-identities.md)
- Next: [066 - Implement seller banking and payout profiles](066-implement-seller-banking-and-payout-profiles.md)
...
- [ ] The implementation of **Build seller storefront branding, policies, and contact controls** is real, reachable where intended, and contains no fake success path.
...
- **Files and migration:** Added migration `20260923070000_seller_branding_policies_contact_controls`, branding/policy/contact fields to seller settings, seller branding upload validation and API, public serializer controls, Admin-safe seller settings persistence, public storefront policy rendering, bilingual copy, OpenAPI updates, and architecture documentation.
- **Decisions:** Seller-authored description and policy text is plain text with bounded lengths; no HTML is accepted. Public email, phone, and pickup address are opt-in and default false. Branding accepts JPEG, PNG, and WebP only, with logo/banner size limits and server-generated public object keys. Existing seller slugs and financial behavior are unchanged.
- **Behavior:** Seller owners and authorized staff can update policies, vacation messaging, contact visibility, and branding assets through seller-scoped APIs. Branding uploads are validated, hashed, stored under `public/store-branding/{sellerId}/...`, audited, and old assets are removed after successful replacement. Verified public stores expose only enabled contact data and allowlisted policy/branding fields.
- **Validation:** `bun run db:generate` and `bun run typecheck` passed. `bun run openapi` passed. Production build and focused branding/settings/public-store tests are required before commit.
- **Security and tenancy:** Branding uploads require authentication, seller tenant matching, settings-management authorization, server-generated keys, file signature validation, and bounded size. Public serialization excludes private seller/KYC/financial fields and respects contact visibility flags. No commission, payout, reward, or Product Point behavior changed.
- **Configuration:** Existing S3-compatible storage configuration and public base URL are required. Production must use private credentials and an approved public asset/CDN policy.
- **Exceptions and next milestone:** SVG, animated images, image dimension decoding, malware scanning, and a separate street-address visibility switch remain deferred. Next: **066 — Implement seller banking and payout profiles**.
