...
title: "Implement S3-compatible product image and video handling"
status: completed
...
# Milestone 085: Implement S3-compatible product image and video handling

## Completion report (2026-09-24)

1. **Files and APIs:** Added product media policy contracts, secure S3-compatible media service, seller upload/list/signed-read/delete routes, OpenAPI multipart schemas, focused tests, and architecture documentation.
2. **Decisions:** Images support JPEG/PNG/WebP/AVIF up to 10 MB; videos support MP4/WebM/QuickTime up to 100 MB; media objects are private and seller/product scoped; signed URLs are short-lived.
3. **Behavior:** Uploads validate MIME and size, enforce seller ownership and product media limits, preserve primary-media behavior, store encrypted private objects, persist metadata, audit uploads/deletions, and issue signed read URLs.
4. **Verification:** TypeScript typecheck, focused product media tests, OpenAPI generation, and `git diff --check` passed. Production build remains to be run in the final validation pass.
5. **Security and integrity:** No application filesystem writes; storage credentials remain server-side; seller tenant checks run before media operations; object keys are not exposed as public URLs; soft deletion and audit records are preserved.
6. **Configuration:** Existing S3-compatible environment variables are required: endpoint, region, bucket, access key, secret key, and path-style setting.
7. **Approved exceptions and next milestone:** Image dimension inspection, transcoding, virus scanning, and asynchronous media processing remain deferred. Next milestone: 086, localized descriptions, specifications, and rich content.

## Outcome

Implement **Implement S3-compatible product image and video handling** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 09, **Products and Media**. The phase purpose is: Deliver production product and variant management with mandatory BDT price and Product Point values.
...
- Previous: [084 - Make BDT price and seller-defined Product Point mandatory](084-make-bdt-price-and-seller-defined-product-point-mandatory.md)
- Next: [086 - Implement localized descriptions, specifications, and rich content](086-implement-localized-descriptions-specifications-and-rich-content.md)
...
- [ ] The implementation of **Implement S3-compatible product image and video handling** is real, reachable where intended, and contains no fake success path.
