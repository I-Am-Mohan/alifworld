...
milestone: 168
phase: 17
phase_name: "Wallet Ledger"
title: "Implement versioned wallet-split templates totaling 100 percent"
status: not-started
currency: BDT
locales: [en-BD, bn-BD]
timezone: Asia/Dhaka
...
# Milestone 168: Implement versioned wallet-split templates totaling 100 percent

## Outcome

Implement **Implement versioned wallet-split templates totaling 100 percent** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 17, **Wallet Ledger**. The phase purpose is: Implement immutable balanced wallet accounting rather than mutable counters.
...
- Previous: [167 - Build deposit and withdrawal request workflows](167-build-deposit-and-withdrawal-request-workflows.md)
- Next: [169 - Implement controlled Admin adjustments with maker-checker approval](169-implement-controlled-admin-adjustments-with-maker-checker-approval.md)
...
- Customer and Seller split templates are separate, effective-dated, and validated to exactly 100%.
- A split creates postings atomically; it does not transfer by updating several counters.
...
Primary expected areas for this phase: `features/wallets, services/ledger, repositories/ledger, app/api/v1/wallets`. The exact existing repository structure wins when it already enforces the same boundaries. Do not create duplicate parallel modules merely to match a suggested path.
...
Phase-level test emphasis: balanced posting, holds, split totals, permissions, concurrency and reconciliation tests.
...
- [ ] The implementation of **Implement versioned wallet-split templates totaling 100 percent** is real, reachable where intended, and contains no fake success path.
