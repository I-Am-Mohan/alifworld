...
title: "Model wallets, points, rewards, ranks, and immutable ledgers"
status: completed
currency: BDT
locales: [en-BD, bn-BD]
timezone: Asia/Dhaka
...
# Milestone 029: Model wallets, points, rewards, ranks, and immutable ledgers

## Outcome

Implement **Model wallets, points, rewards, ranks, and immutable ledgers** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 03, **Data Architecture**. The phase purpose is: Build a production PostgreSQL/Prisma model that preserves history, tenant boundaries, and financial traceability.
...
- Previous: [028 - Model payments, refunds, commissions, settlements, and payouts](028-model-payments-refunds-commissions-settlements-and-payouts.md)
- Next: [030 - Create migration, seed, and database-dictionary workflows](030-create-migration-seed-and-database-dictionary-workflows.md)
...
- Model ledger accounts, entries, postings, Point events, reward definitions, rule versions, period settlements, leaderboard snapshots, commission allocations, and reversal relationships.
- Use database constraints to protect balance and uniqueness invariants.
- Never update a balance without balanced ledger postings.
- Express holds and available balance explicitly; do not hide pending amounts in arithmetic.
- Make every posting idempotent and link reversals instead of deleting history.
...
Primary expected areas for this phase: `prisma/schema.prisma, prisma/migrations, prisma/seed.ts, repositories`. The exact existing repository structure wins when it already enforces the same boundaries. Do not create duplicate parallel modules merely to match a suggested path.
...
Phase-level test emphasis: migration tests, constraints, index inspection, seed idempotency.
...
- [x] The implementation of **Model wallets, points, rewards, ranks, and immutable ledgers** is real, reachable where intended, and contains no fake success path.
