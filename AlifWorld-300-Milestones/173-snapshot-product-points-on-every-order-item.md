...
milestone: 173
phase: 18
phase_name: "Product Points Engine"
title: "Snapshot Product Points on every order item"
status: not-started
currency: BDT
locales: [en-BD, bn-BD]
timezone: Asia/Dhaka
...
# Milestone 173: Snapshot Product Points on every order item

## Outcome

Implement **Snapshot Product Points on every order item** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 18, **Product Points Engine**. The phase purpose is: Treat Product Points as an independent, immutable, order-snapshotted unit with reversible ledger events.
...
- Previous: [172 - Enforce mandatory seller-defined Product Points on sellable products](172-enforce-mandatory-seller-defined-product-points-on-sellable-products.md)
- Next: [174 - Generate Points only at the configured eligible order status](174-generate-points-only-at-the-configured-eligible-order-status.md)
...
- Store productPointSnapshot, eligibleQuantity, generatedPoints, pointRuleVersionId, sellerId, customerId, productId, variantId, and source order item.
- Changing a product later affects only future order items.
...
Primary expected areas for this phase: `features/points, services/points, workers/points, app/api/v1/points`. The exact existing repository structure wins when it already enforces the same boundaries. Do not create duplicate parallel modules merely to match a suggested path.
...
Phase-level test emphasis: mandatory value, snapshot, eligibility, reversal, replay, BDT example and reconciliation tests.
...
- [ ] The implementation of **Snapshot Product Points on every order item** is real, reachable where intended, and contains no fake success path.
