...
title: "Implement curated and rule-based collections"
status: completed
...
# Milestone 073: Implement curated and rule-based collections

## Completion report (2026-09-23)

1. **Files and migration:** Added collection Prisma models and migration, collection validators/types, repository, service, REST routes, architecture documentation, focused unit tests, collection authorization alias, and OpenAPI paths/schemas.
2. **Decisions:** Curated collections use explicit unique memberships; rule-based collections accept only bounded typed criteria; public visibility requires active published status; administrators own collection mutations; product points and financial behavior remain independent and unchanged.
3. **Behavior:** Added public collection listing/detail routes and protected admin create/update/publish/archive/membership replacement routes. Mutations use optimistic versions and audit records.
4. **Verification:** `bunx prisma validate`, `bunx prisma generate`, focused Bun tests, `bun run typecheck`, `bun run openapi`, `git diff --check`, and `bun run build:local` passed. The build retains only pre-existing lint warnings for image optimization and hook dependencies.
5. **Security and integrity:** Admin authorization, soft-delete filtering, duplicate membership rejection, bounded rule parsing, product existence checks, approved-brand and active-category governance, active-seller filtering, optimistic concurrency, and public draft exclusion are implemented.
6. **Configuration:** No new credentials are required. Database migration deployment is required in each environment.
7. **Exceptions and next step:** No background indexing or cache invalidation is needed for this bounded catalog query path. Next milestone: 074, attributes, values, and variant option sets.



Implement **Implement curated and rule-based collections** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 08, **Catalog Foundations**. The phase purpose is: Build governed taxonomy and catalog primitives usable by sellers, customers, Admin, and search.
...
- Previous: [072 - Implement brands and brand approval](072-implement-brands-and-brand-approval.md)
- Next: [074 - Implement attributes, values, and variant option sets](074-implement-attributes-values-and-variant-option-sets.md)
...
- [ ] The implementation of **Implement curated and rule-based collections** is real, reachable where intended, and contains no fake success path.
