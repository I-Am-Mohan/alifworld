...
title: "Implement BDT and multi-currency-safe monetary primitives"
status: completed
...
# Milestone 054: Implement BDT and multi-currency-safe monetary primitives

## Outcome

Implement **Implement BDT and multi-currency-safe monetary primitives** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 06, **Bangladesh Localization**. The phase purpose is: Make Bangladesh the first operational market while keeping locale, currency, and geography extensible.
...
- Previous: [053 - Model localizable catalog and CMS content](053-model-localizable-catalog-and-cms-content.md)
- Next: [055 - Standardize Asia/Dhaka time and business-period boundaries](055-standardize-asia-dhaka-time-and-business-period-boundaries.md)
...
- Default currency is BDT; retain an ISO currency column for extensibility.
- Use integer poisha in application contracts; never JavaScript floating-point arithmetic for money.
- Format `৳` by locale but do not store formatted strings.
...
- [ ] The implementation of **Implement BDT and multi-currency-safe monetary primitives** is real, reachable where intended, and contains no fake success path.
...
1. **Files and migrations changed**
   - Updated `src/shared/types/domain-terms.ts` so `Poisha` is a branded `bigint` and decimal conversion is exact.
   - Updated `src/shared/utils/currency.ts` and `src/shared/utils/localization.ts` to avoid floating-point conversion for poisha formatting and parsing.
   - Updated `src/shared/types/currency.ts` with supported currency codes, minor-unit metadata, validation, and exact minor-unit formatting.
   - Updated `src/validators/payment.validator.ts` to validate currency codes.
   - Updated catalog tax types/service to calculate tax with integer poisha and basis points.
   - Updated currency and tax unit tests.
   - No Prisma migration was required; existing persistence already stores financial amounts as BigInt poisha fields.

2. **Decisions and source authority**
   - Preserved BDT as the launch currency and `1 BDT = 100 poisha`.
   - Changed the canonical TypeScript monetary primitive from branded `number` to branded `bigint` to satisfy the locked no-floating-point requirement.
   - Kept Product Points as a separate non-convertible integer domain value.
   - Used integer basis points for tax arithmetic while preserving the existing configured tax-rate input shape.
   - Preserved existing UI/configuration compatibility for legacy major-unit number formatting at non-financial display boundaries.

3. **Behavior added**
   - Exact parsing rejects malformed BDT input and more than two fractional digits.
   - Large poisha values remain exact beyond JavaScript safe-integer range.
   - Supported currency metadata covers BDT, USD, INR, EUR, GBP, AED, SAR, and JPY with explicit minor-unit scales.
   - Tax calculation now uses BigInt multiplication and half-up integer rounding instead of floating-point monetary arithmetic.
   - No new API, UI, worker, cache, search, or provider behavior was required.

4. **Tests and commands**
   - Focused monetary and tax tests: 17 passed, 0 failed.
   - `bun run typecheck`: passed.
   - `bun run build:local`: passed.
   - Build output retains the repository’s existing ESLint warnings; compilation and type validation passed.

5. **Security, tenancy, localization, and financial-integrity checks**
   - No authorization or tenant boundary was weakened.
   - BDT formatting remains locale-aware for `en-BD` and `bn-BD`.
   - Poisha arithmetic no longer converts BigInt values through `Number` in the updated canonical paths.
   - Product Points remain independent from monetary values.
   - Tax totals and line-item totals are now integer-poisha snapshots.

6. **Configuration or credentials still required**
   - No new environment variables or credentials are required.
   - Additional currencies require an explicit entry in `CURRENCY_MINOR_UNITS` and corresponding symbol/configuration before activation.

7. **Approved exceptions, unresolved gates, and exact next milestone**
   - Existing full-suite and lint warnings remain documented repository-wide exceptions and were not introduced by this change.
   - Existing legacy callers that pass major-unit JavaScript numbers to display-only currency configuration remain supported; new financial code must use BigInt poisha.
   - Exact next milestone: **055 — Standardize Asia/Dhaka time and business-period boundaries**.
