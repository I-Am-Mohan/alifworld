...
title: "Implement seller suspension, restriction, and reactivation"
status: completed
...
# Milestone 068: Implement seller suspension, restriction, and reactivation

## Outcome

Implement **Implement seller suspension, restriction, and reactivation** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 07, **Seller Lifecycle**. The phase purpose is: Onboard, approve, operate, and govern sellers and their staff safely.
...
- Previous: [067 - Implement seller tax, shipping, order, and notification defaults](067-implement-seller-tax-shipping-order-and-notification-defaults.md)
- Next: [069 - Build seller staff administration and activity visibility](069-build-seller-staff-administration-and-activity-visibility.md)
...
- [ ] The implementation of **Implement seller suspension, restriction, and reactivation** is real, reachable where intended, and contains no fake success path.
...
- **Files and migration:** Added seller lifecycle history persistence through migration `20260923100000_seller_lifecycle_suspension`, explicit lifecycle transition validation, transactional suspension/restriction/reactivation service operations, Admin lifecycle API and controls, canonical audit/outbox events, OpenAPI documentation, architecture documentation, and focused tests.
- **Decisions:** `VERIFIED` sellers may become `RESTRICTED` or `SUSPENDED`; either restricted/suspended state may reactivate to `VERIFIED`, with `RESTRICTED ↔ SUSPENDED` allowed for administrative correction. Every mutation requires a reason and optimistic version. Public storefront access remains verified-only.
- **Behavior:** `POST /api/v1/admin/sellers/{id}/lifecycle` supports `RESTRICT`, `SUSPEND`, and `REACTIVATE`. Transitions are atomic, append lifecycle history, audit the reason/status change, and emit outbox events. Seller users cannot invoke the Admin route.
- **Validation:** `bun run db:generate`, `bun run typecheck`, focused lifecycle tests, `bun run openapi`, `bun run build:local`, and `git diff --check` passed. Existing repository lint warnings remain.
- **Security and tenancy:** Lifecycle transitions require existing seller administration permissions, seller IDs are route-scoped, stale versions fail closed, and reasons are bounded. No financial, payout, commission, reward, or inventory calculations changed.
- **Configuration:** No new environment variables are required. Existing JWT, authorization, PostgreSQL, audit, and outbox configuration remains required.
- **Exceptions and next milestone:** Full operational blocking across every seller order/inventory path is deferred to the relevant later domain workflows; public visibility already excludes non-verified sellers. Next: **069 — Build seller staff administration and activity visibility**.
