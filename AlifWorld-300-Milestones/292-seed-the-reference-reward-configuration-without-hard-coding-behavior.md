...
milestone: 292
phase: 30
phase_name: "Launch, Handover, and Continuous Improvement"
title: "Seed the reference reward configuration without hard-coding behavior"
status: not-started
currency: BDT
locales: [en-BD, bn-BD]
timezone: Asia/Dhaka
...
# Milestone 292: Seed the reference reward configuration without hard-coding behavior

## Outcome

Implement **Seed the reference reward configuration without hard-coding behavior** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 30, **Launch, Handover, and Continuous Improvement**. The phase purpose is: Seed, migrate, verify, launch, and hand over an operable Bangladesh production system.
...
- Previous: [291 - Seed initial roles, permissions, and the forced-change Admin account](291-seed-initial-roles-permissions-and-the-forced-change-admin-account.md)
- Next: [293 - Seed Bangladesh geography and localized baseline content](293-seed-bangladesh-geography-and-localized-baseline-content.md)
...
- Seed reference values as versioned inactive/draft configurations where a gate remains unresolved.
- Never compile the percentages into service code.
...
- Use upserts and stable keys so reruns are safe.
- Create `contact@alifworld.com.bd` from environment-controlled seed credentials and force password change.
...
Primary expected areas for this phase: `prisma/seed.ts, docs, runbooks, release evidence, handover`. The exact existing repository structure wins when it already enforces the same boundaries. Do not create duplicate parallel modules merely to match a suggested path.
...
Phase-level test emphasis: seed idempotency, migration reconciliation, full regression, DR rehearsal and acceptance sign-off.
...
- [ ] The implementation of **Seed the reference reward configuration without hard-coding behavior** is real, reachable where intended, and contains no fake success path.
