...
title: "Build the seller application workflow"
status: completed
...
# Milestone 061: Build the seller application workflow

## Outcome

Implement **Build the seller application workflow** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 07, **Seller Lifecycle**. The phase purpose is: Onboard, approve, operate, and govern sellers and their staff safely.
...
- Previous: [060 - Complete localization fallback, translation-admin, and QA tooling](060-complete-localization-fallback-translation-admin-and-qa-tooling.md)
- Next: [062 - Implement secure seller KYC and document uploads](062-implement-secure-seller-kyc-and-document-uploads.md)
...
- [ ] The implementation of **Build the seller application workflow** is real, reachable where intended, and contains no fake success path.
...
- **Files and migration:** Added the `SellerApplication` and `SellerApplicationReview` Prisma models and migration `20260923040000_add_seller_application_workflow`; added seller application types, validation, repository, transactional service, protected seller and Admin routes, seller onboarding UI, Admin review UI, architecture documentation, bilingual translations, and focused tests.
- **Decisions:** Applications begin as applicant-owned drafts and become seller tenants only on transactional approval. Review states are `DRAFT`, `SUBMITTED`, `UNDER_REVIEW`, `CHANGES_REQUESTED`, `APPROVED`, `REJECTED`, and `WITHDRAWN`. Approval/rejection/request-changes require platform `sellers:verify`; seller roles are explicitly denied review actions. Private KYC upload and document approval remain in Milestone 062.
- **Behavior:** Applicants can create, edit, save, submit, and resubmit applications after requested changes. Platform reviewers can list submitted applications, mark them under review, request changes, reject, or approve. Approval creates the seller, default settings, owner staff membership, scoped owner role, audit entry, review history, and outbox event in one transaction.
- **Validation:** `bun run db:generate` passed. `bun run typecheck` passed. Focused seller application tests cover validation, review-reason requirements, seller-role denial, and platform reviewer authorization. OpenAPI/build validation is required before commit.
- **Security and tenancy:** Applicant routes use authenticated user ownership; seller-scoped repository methods place `sellerId` in the database `where` clause; review routes require `sellers:verify`; optimistic versions prevent stale updates and review races; no financial, payout, commission, or Product Point behavior was introduced.
- **Configuration:** No new environment variables or credentials are required. Existing PostgreSQL, JWT, Admin authorization, audit, and outbox configuration is required.
- **Exceptions and next milestone:** Persistent KYC document handling is intentionally deferred to **062 — Implement secure seller KYC and document uploads**. The non-interactive Prisma migration command could not run against the remote development database, so the reviewed migration SQL was generated with Prisma diff and committed manually.
