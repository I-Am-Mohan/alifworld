...
title: "Model localizable catalog and CMS content"
status: completed
...
# Milestone 053: Model localizable catalog and CMS content

## Outcome

Implement **Model localizable catalog and CMS content** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 06, **Bangladesh Localization**. The phase purpose is: Make Bangladesh the first operational market while keeping locale, currency, and geography extensible.
...
- Previous: [052 - Create English and Bangla translation catalogs](052-create-english-and-bangla-translation-catalogs.md)
- Next: [054 - Implement BDT and multi-currency-safe monetary primitives](054-implement-bdt-and-multi-currency-safe-monetary-primitives.md)
...
- [ ] The implementation of **Model localizable catalog and CMS content** is real, reachable where intended, and contains no fake success path.
...
1. **Files and migrations changed**
   - Added normalized catalog localization and CMS Prisma models.
   - Added migration `prisma/migrations/20260922010000_model_localized_catalog_and_cms_content/migration.sql`.
   - Added localization contracts, repositories, service, CMS route handlers, OpenAPI metadata, unit tests, and architecture documentation.
   - Preserved existing Bangla compatibility fields such as `nameBn`, `titleBn`, and `descriptionBn`.

2. **Decisions and source authority**
   - Used the existing modular-monolith architecture and repository/service boundaries required by this milestone.
   - Added locale fallback in the order exact locale, language-only locale, `bn-BD`, then `bn`.
   - CMS content uses draft/review/published/archived states, soft deletion fields, versioning, and locale-specific JSON content.
   - No financial, Point, reward, commission, or currency rules were introduced.

3. **Behavior added**
   - Public endpoint: `GET /api/v1/content/[slug]?locale=...` returns published localized content with fallback.
   - Admin endpoints: `POST /api/v1/admin/content` and `PATCH /api/v1/admin/content/[id]`.
   - Admin mutations require authenticated `ADMIN` or `SUPER_ADMIN` roles.
   - CMS updates enforce optimistic concurrency and write audit records.
   - No UI, worker, cache, search, or external provider behavior was added because it is not required for this data-model milestone.

4. **Tests and commands**
   - `prisma validate`: passed.
   - `prisma generate`: passed.
   - Focused localization and translation tests: 7 passed, 0 failed.
   - `bun run typecheck`: passed.
   - `bun run openapi`: passed.
   - `bun run build:local`: passed.
   - Existing build lint warnings remain; the repository-wide lint gate was not made green because of pre-existing warnings.

5. **Security, tenancy, localization, and financial-integrity checks**
   - Server-side authentication and role authorization are enforced for CMS mutations.
   - Public reads filter out deleted and unpublished content.
   - Locale validation and fallback tests cover `en-BD`, `bn-BD`, and short language forms.
   - Existing seller/product financial and Point behavior was not changed.
   - CMS writes include audit logging and optimistic version checks.

6. **Configuration or credentials required**
   - No new credentials or environment variables are required.
   - The configured remote development database must apply the checked-in migration through the normal migration deployment workflow.

7. **Approved exceptions, unresolved gates, and next milestone**
   - `prisma migrate dev` could not run in this non-interactive environment against the configured remote database; the reviewed migration SQL is checked in and Prisma schema validation passed.
   - Full repository lint/test/e2e gates retain previously documented unrelated failures and warnings.
   - Exact next milestone: **054 — Implement BDT and multi-currency-safe monetary primitives**.
