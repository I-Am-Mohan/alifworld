...
title: "Standardize Asia/Dhaka time and business-period boundaries"
status: completed
...
# Milestone 055: Standardize Asia/Dhaka time and business-period boundaries

## Outcome

Implement **Standardize Asia/Dhaka time and business-period boundaries** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 06, **Bangladesh Localization**. The phase purpose is: Make Bangladesh the first operational market while keeping locale, currency, and geography extensible.
...
- Previous: [054 - Implement BDT and multi-currency-safe monetary primitives](054-implement-bdt-and-multi-currency-safe-monetary-primitives.md)
- Next: [056 - Model Bangladesh phone, address, and geographic hierarchy data](056-model-bangladesh-phone-address-and-geographic-hierarchy-data.md)
...
- [ ] The implementation of **Standardize Asia/Dhaka time and business-period boundaries** is real, reachable where intended, and contains no fake success path.
...
1. **Files and migrations changed**
   - Updated `src/shared/utils/date.ts` with canonical Dhaka local ISO output and deterministic daily, weekly, monthly, and yearly period boundaries.
   - Added configurable weekly start day and local business-day start hour/minute.
   - Added fixed-clock tests in `tests/unit/date-and-timezone.test.ts`.
   - No Prisma migration was required; persisted timestamps remain UTC instants while business boundaries are derived in `Asia/Dhaka`.

2. **Decisions and source authority**
   - Preserved `Asia/Dhaka` as the sole business timezone.
   - Period calculations use local calendar dates in Dhaka and convert boundaries back to UTC `Date` instants for persistence and queries.
   - Weekly boundaries accept an explicit `weekStartsOn` configuration; Monday is the documented engineering default, avoiding an invented business rule.
   - Business start time is configurable and defaults to local midnight.

3. **Behavior added**
   - `getDhakaIsoString()` now returns a stable local representation rather than a locale-dependent string.
   - `getDhakaStartOfDay()` supports configurable local start times.
   - Added `getDhakaEndOfDay()`.
   - Added `getDhakaPeriodBounds()` for daily, weekly, monthly, and yearly periods with inclusive end timestamps.
   - No new REST route, UI, worker, cache, search, or provider behavior was required.

4. **Tests and commands**
   - Dhaka date/time tests: 6 passed, 0 failed.
   - Existing locale/date regression tests: 32 passed, 0 failed.
   - `bun run typecheck`: passed.
   - `bun run build:local`: passed.
   - Existing repository ESLint warnings remain documented and were not introduced by this change.

5. **Security, tenancy, localization, and financial-integrity checks**
   - No authorization or tenant boundary was changed.
   - Date formatting remains locale-aware for `en-BD` and `bn-BD`.
   - Boundary functions use fixed clocks in tests and return UTC instants suitable for database range queries.
   - No monetary, Point, reward, or financial calculation behavior was changed.

6. **Configuration or credentials still required**
   - No new credentials or environment variables are required.
   - Callers defining weekly or non-midnight business periods must supply the approved configuration values.

7. **Approved exceptions, unresolved gates, and exact next milestone**
   - Full repository lint/test/e2e gates retain previously documented unrelated warnings and failures.
   - The weekly start day remains configuration-driven pending final business approval.
   - Exact next milestone: **056 — Model Bangladesh phone, address, and geographic hierarchy data**.
