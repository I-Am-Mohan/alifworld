...
title: "Implement catalog permissions and product approval workflow"
status: completed
...
# Milestone 076: Implement catalog permissions and product approval workflow

## Completion report (2026-09-23)

1. **Files and migration:** Added `ProductApprovalRequest` and `ProductStatusHistory` models with migration `20260923150000_product_approval_workflow`; approval validators, repository, service, routes, catalog permissions, OpenAPI paths/schemas, focused tests, and architecture documentation.
2. **Decisions:** Sellers submit owned drafts but cannot approve or publish; Admin approval and publication are separate transitions; rejected products can be revised and resubmitted; approval transitions are versioned and auditable.
3. **Behavior:** Added readiness validation for seller/category/brand/status/BDT price/Product Points/media/required attributes/variant options, idempotent seller submission, Admin pending queue, approve/reject/publish/archive actions, review history, transactional audit/outbox records, and seller publication denial in `CatalogPolicy`.
4. **Verification:** Prisma validate/generate, TypeScript typecheck, OpenAPI generation, focused approval/catalog/tax/localization/SEO tests, and `git diff --check` passed.
5. **Security and integrity:** Seller tenant ownership is checked in the service and policy layers; Admin approval actions require catalog approval/publication permissions; optimistic versions prevent lost updates; status history is immutable; no financial or Product Point calculation behavior changed.
6. **Configuration:** No new credentials are required. Deploy migration `20260923150000_product_approval_workflow` before enabling approval persistence.
7. **Approved exceptions and next milestone:** The legacy `ProductService.publishProduct()` remains for backward compatibility with existing callers; new workflow routes enforce the governed approval path. Full UI integration and deeper product authoring are deferred to later catalog milestones. Next milestone: 077, seller catalog onboarding templates and guidance.

## Outcome

Implement **Implement catalog permissions and product approval workflow** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 08, **Catalog Foundations**. The phase purpose is: Build governed taxonomy and catalog primitives usable by sellers, customers, Admin, and search.
...
- Previous: [075 - Add taxonomy translation and SEO controls](075-add-taxonomy-translation-and-seo-controls.md)
- Next: [077 - Create seller catalog onboarding templates and guidance](077-create-seller-catalog-onboarding-templates-and-guidance.md)
...
- [ ] The implementation of **Implement catalog permissions and product approval workflow** is real, reachable where intended, and contains no fake success path.
