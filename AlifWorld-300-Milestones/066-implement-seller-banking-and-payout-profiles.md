...
title: "Implement seller banking and payout profiles"
status: completed
...
# Milestone 066: Implement seller banking and payout profiles

## Outcome

Implement **Implement seller banking and payout profiles** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 07, **Seller Lifecycle**. The phase purpose is: Onboard, approve, operate, and govern sellers and their staff safely.
...
- Previous: [065 - Build seller storefront branding, policies, and contact controls](065-build-seller-storefront-branding-policies-and-contact-controls.md)
- Next: [067 - Implement seller tax, shipping, order, and notification defaults](067-implement-seller-tax-shipping-order-and-notification-defaults.md)
...
- [ ] The implementation of **Implement seller banking and payout profiles** is real, reachable where intended, and contains no fake success path.
...
- **Files and migration:** Added migration `20260923080000_seller_payout_profiles`, encrypted payout-profile crypto, payout-profile model/types/validator/repository/service, protected seller payout-profile API, disabled fake payout execution UI, OpenAPI documentation, architecture documentation, translations, and focused security tests.
- **Decisions:** Raw account, routing, and account-title values are encrypted with AES-256-GCM and never returned. Only last-four digits, provider, status, and safe metadata are exposed. Seller owners may replace profiles; seller staff cannot mutate them by default. Payout execution and settlement approval remain deferred and no financial formula was invented.
- **Behavior:** `GET/PUT /api/v1/seller/payout-profile` supports seller-scoped masked reads and OCC-protected primary-profile replacement. Duplicate account fingerprints are rejected. Profile replacement deactivates the previous primary profile and emits audit records. The previous finance-page fake BEFTN success path is disabled.
- **Validation:** `bun run db:generate`, `bun run typecheck`, `bun test tests/unit/seller-payout-profile.test.ts`, `bun run openapi`, `bun run build:local`, and `git diff --check` passed. The build reports existing repository lint warnings only.
- **Security and tenancy:** Payout queries require seller scope; seller staff mutation is denied; ciphertext is server-only; API responses exclude ciphertext and full banking values; audit metadata contains only safe masked fields. No payout is initiated by this milestone.
- **Configuration:** Production requires a managed `PAYOUT_PROFILE_ENCRYPTION_KEY` of at least 32 characters, with rotation/key-management procedures. Existing PostgreSQL, JWT, and authorization configuration remains required.
- **Exceptions and next milestone:** Existing settlement/payout execution repositories remain out of scope and should not be used as an approval or disbursement path until later financial milestones harden them. Next: **067 — Implement seller tax, shipping, order, and notification defaults**.
...
- **Decisions:** Raw account, routing, and account-title values are encrypted with AES-256-GCM and never returned. Only last-four digits, provider, status, and safe metadata are exposed. Seller owners may replace profiles; seller staff cannot mutate them by default. Payout execution and settlement approval remain deferred and no financial formula was invented.
- **Behavior:** `GET/PUT /api/v1/seller/payout-profile` supports seller-scoped masked reads and OCC-protected primary-profile replacement. Duplicate account fingerprints are rejected. Profile replacement deactivates the previous primary profile and emits audit records. The previous finance-page fake BEFTN success path is disabled.
- **Validation:** `bun run db:generate` and `bun run typecheck` passed. `bun run openapi` passed. Focused payout-profile security tests cover encryption, random ciphertext, fingerprints, last-four masking, and input validation. Production build validation is required before commit.
- **Security and tenancy:** Payout queries require seller scope; seller staff mutation is denied; ciphertext is server-only; API responses exclude ciphertext and full banking values; audit metadata contains only safe masked fields. No payout is initiated by this milestone.
- **Configuration:** Production requires a managed `PAYOUT_PROFILE_ENCRYPTION_KEY` of at least 32 characters, with rotation/key-management procedures. Existing PostgreSQL, JWT, and authorization configuration remains required.
- **Exceptions and next milestone:** Existing settlement/payout execution repositories remain out of scope and should not be used as an approval or disbursement path until later financial milestones harden them. Next: **067 — Implement seller tax, shipping, order, and notification defaults**.
