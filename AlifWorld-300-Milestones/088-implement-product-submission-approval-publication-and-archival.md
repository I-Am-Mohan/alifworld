...
title: "Implement product submission, approval, publication, and archival"
status: completed
...
# Milestone 088: Implement product submission, approval, publication, and archival

## Completion report (2026-09-23)

1. **Files and workflow:** Hardened the existing ProductApprovalService readiness contract with title, description, slug, category, brand, seller, BDT price, Product Point, media, attributes, and variant checks; added focused lifecycle tests and architecture documentation.
2. **Decisions:** The canonical lifecycle is `DRAFT → PENDING_APPROVAL → APPROVED → PUBLISHED → ARCHIVED`; rejected products can be revised and resubmitted; Seller users cannot approve, publish, or archive; Admin actions remain permission-protected.
3. **Behavior:** Existing seller submission/readiness and Admin approve/reject/publish/archive APIs are the authoritative workflow. Optimistic versions, immutable status history, audit/outbox events, idempotent submission, and slug history are preserved.
4. **Verification:** TypeScript typecheck, publication workflow tests, existing approval/product tests, OpenAPI generation, and `git diff --check` passed.
5. **Security and integrity:** Seller ownership is checked before submission; publication and archival require Admin policy decisions; readiness validates BDT price and independent Product Points; no historical order snapshots are altered.
6. **Configuration:** No new credentials or migrations are required. Existing approval workflow migrations must be deployed.
7. **Approved exceptions and next milestone:** Inventory stock-policy enforcement remains delegated to the inventory foundation rather than inventing a stock rule here. Next milestone: 089, product version history and audit visibility.

## Outcome

Implement **Implement product submission, approval, publication, and archival** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 09, **Products and Media**. The phase purpose is: Deliver production product and variant management with mandatory BDT price and Product Point values.
...
- Previous: [087 - Implement product weight, dimensions, tax, and shipping attributes](087-implement-product-weight-dimensions-tax-and-shipping-attributes.md)
- Next: [089 - Implement product version history and audit visibility](089-implement-product-version-history-and-audit-visibility.md)
...
- [ ] The implementation of **Implement product submission, approval, publication, and archival** is real, reachable where intended, and contains no fake success path.
