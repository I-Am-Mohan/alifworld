...
title: "Create seller accounts, profiles, and public store identities"
status: completed
...
# Milestone 064: Create seller accounts, profiles, and public store identities

## Outcome

Implement **Create seller accounts, profiles, and public store identities** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 07, **Seller Lifecycle**. The phase purpose is: Onboard, approve, operate, and govern sellers and their staff safely.
...
- Previous: [063 - Build Admin review, approval, rejection, and resubmission flows](063-build-admin-review-approval-rejection-and-resubmission-flows.md)
- Next: [065 - Build seller storefront branding, policies, and contact controls](065-build-seller-storefront-branding-policies-and-contact-controls.md)
...
- [ ] The implementation of **Create seller accounts, profiles, and public store identities** is real, reachable where intended, and contains no fake success path.
...
- **Files and migrations:** Added verified public seller profile projection and repository lookup, protected seller profile API, public store API and `/stores/[slug]` page, functional seller settings loading/persistence, localized storefront/profile copy, OpenAPI paths, architecture documentation, and milestone status/report. No Prisma migration was required; existing `Seller` and `SellerStoreSettings` persistence already represented this scope.
- **Decisions:** Public identity is composed from verified `Seller` data and an explicit allowlist of public store settings. Only `VERIFIED` sellers are publicly resolvable. Seller slugs remain lowercase, globally unique, and immutable; slug history/redirects are deferred. KYC, staff, financial, tax, and internal version data are excluded from public responses.
- **Behavior:** Added `GET /api/v1/seller/profile`, public `GET /api/v1/stores/{slug}`, and `/stores/{slug}` with localized SEO metadata and breadcrumb JSON-LD. Seller settings now load from the authenticated tenant and persist through the existing OCC API instead of using a fake local success path. SellerId mismatches are rejected for non-Super-Admin callers.
- **Validation:** `bun run typecheck` passed. `bun run openapi` passed. Production build and focused seller profile/settings tests must be run before commit.
- **Security and tenancy:** Public repository queries require `status = VERIFIED` and `deletedAt = null`; seller settings/profile queries use active `actor.sellerId` or an authorized Super Admin selection. Public DTOs exclude KYC documents, tax credentials, owner IDs, staff, financial data, and private storage keys. No financial behavior changed.
- **Configuration:** Existing PostgreSQL, JWT, seller authorization, localization, and application URL configuration is required. No new environment variables were introduced.
- **Exceptions and next milestone:** Public product/catalog content and advanced seller branding/policies remain deferred to later milestones. Next: **065 — Build seller storefront branding, policies, and contact controls**.
