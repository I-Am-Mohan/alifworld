...
title: "Complete product CRUD across API, Seller UI, Admin UI, and tests"
status: completed
...
# Milestone 090: Complete product CRUD across API, Seller UI, Admin UI, and tests

## Outcome

Implement **Complete product CRUD across API, Seller UI, Admin UI, and tests** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 09, **Products and Media**. The phase purpose is: Deliver production product and variant management with mandatory BDT price and Product Point values.
...
- Previous: [089 - Implement product version history and audit visibility](089-implement-product-version-history-and-audit-visibility.md)
- Next: [091 - Define BDT pricing and immutable price snapshots](091-define-bdt-pricing-and-immutable-price-snapshots.md)
...
- Use `/api/v1`, bearer tokens for mobile clients, stable JSON envelopes, and correct HTTP semantics.
- Return machine-readable error codes and never leak stack traces.
...
- [ ] The implementation of **Complete product CRUD across API, Seller UI, Admin UI, and tests** is real, reachable where intended, and contains no fake success path.
...
## Completion report (2026-09-23)

1. **Files changed:** Hardened product repository/service scoping and deletion, added `ProductVariantService`, seller variant CRUD routes, seller product deletion, Admin product CRUD route and edit page, seller delete/edit actions, validators, OpenAPI metadata, focused CRUD tests, and architecture documentation.
2. **Decisions:** Kept soft deletion and optimistic concurrency, kept approval/publication as separate lifecycle workflows, and made variant/media subresources explicit owners rather than silently accepting nested PATCH fields.
3. **API/UI behavior:** Added seller variant list/create/read/update/delete, seller product DELETE, Admin product GET/PATCH/DELETE, validated collection queries, Admin edit UI, and Seller edit/delete affordances.
4. **Verification:** Focused milestone tests passed: 4 passed, 0 failed. TypeScript diagnostics and `bun run typecheck` passed. OpenAPI generation and clean production build are required final gates.
5. **Security and integrity:** Repository queries apply seller scope, seller access is checked server-side, Admin routes use catalog policy checks, SKU/barcode uniqueness is enforced, BDT prices remain safe integers, Product Point remains independent, and deletion is version-protected and audited.
6. **Configuration:** No new credentials or environment variables are required; no schema migration was added.
7. **Exceptions and next milestone:** Existing repository lint warnings and unrelated baseline test failures remain outside this milestone. Next milestone: 091, BDT pricing and immutable price snapshots.
