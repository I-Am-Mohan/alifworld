...
title: "Build Admin review, approval, rejection, and resubmission flows"
status: completed
...
# Milestone 063: Build Admin review, approval, rejection, and resubmission flows

## Outcome

Implement **Build Admin review, approval, rejection, and resubmission flows** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 07, **Seller Lifecycle**. The phase purpose is: Onboard, approve, operate, and govern sellers and their staff safely.
...
- Previous: [062 - Implement secure seller KYC and document uploads](062-implement-secure-seller-kyc-and-document-uploads.md)
- Next: [064 - Create seller accounts, profiles, and public store identities](064-create-seller-accounts-profiles-and-public-store-identities.md)
...
- [ ] The implementation of **Build Admin review, approval, rejection, and resubmission flows** is real, reachable where intended, and contains no fake success path.
...
- **Files and migrations:** Added the seller review transition contract, application detail route, review idempotency migration `20260923060000_strengthen_seller_review_idempotency`, Admin application/KYC review workspace behavior, standardized audit/outbox events, atomic KYC review guards, OpenAPI updates, architecture documentation, translations, and focused tests.
- **Decisions:** Applications must move from `SUBMITTED` to `UNDER_REVIEW` before approval, rejection, or change requests. Decision outcomes require a reviewer reason. `CHANGES_REQUESTED` returns control to the applicant for edit/resubmit; approved, rejected, and withdrawn applications are terminal in this milestone. KYC review remains a separate compliance surface from application activation because the previous milestone created that boundary.
- **Behavior:** Admins with `sellers:verify` can inspect full application history, reserve applications for review, approve, reject, or request changes. The Admin page now includes pending KYC review, secure document view, verification, and rejection controls. Review retries can use `Idempotency-Key`; application version checks remain mandatory.
- **Validation:** `bun run db:generate`, `bun run typecheck`, focused application/KYC/tenant tests passed with 18 tests and 35 assertions, and `bun run openapi` passed. Production build validation is required before commit.
- **Security and tenancy:** Seller roles cannot perform review decisions. Admin KYC review requires the platform reviewer permission and atomically checks document status/version. KYC verification metadata is cleared on rejection. Audit actions use the canonical audit catalog and review decisions emit outbox events. No financial, payout, commission, reward, or Product Point behavior changed.
- **Configuration:** No new environment variables are required. Existing JWT, authorization, PostgreSQL, outbox, audit, and S3-compatible storage configuration remains required.
- **Exceptions and next milestone:** Application approval still creates the seller tenant before any later public identity/profile work; KYC and application approval are intentionally separate based on the accepted Milestones 061–062 boundary. Next: **064 — Create seller accounts, profiles, and public store identities**.
