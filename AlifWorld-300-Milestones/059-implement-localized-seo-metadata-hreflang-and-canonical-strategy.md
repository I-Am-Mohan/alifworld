...
title: "Implement localized SEO metadata, hreflang, and canonical strategy"
status: completed
...
# Milestone 059: Implement localized SEO metadata, hreflang, and canonical strategy

## Outcome

Implement **Implement localized SEO metadata, hreflang, and canonical strategy** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 06, **Bangladesh Localization**. The phase purpose is: Make Bangladesh the first operational market while keeping locale, currency, and geography extensible.
...
- Previous: [058 - Localize validation, errors, notifications, and transactional content](058-localize-validation-errors-notifications-and-transactional-content.md)
- Next: [060 - Complete localization fallback, translation-admin, and QA tooling](060-complete-localization-fallback-translation-admin-and-qa-tooling.md)
...
- [ ] The implementation of **Implement localized SEO metadata, hreflang, and canonical strategy** is real, reachable where intended, and contains no fake success path.
...
1. **Files changed:** Added `src/shared/seo/metadata.ts` and JSON-LD rendering, localized canonical/hreflang helpers, root/storefront metadata integration, product and listing metadata layouts, private-route noindex layouts, `robots.txt`, `sitemap.ts`, SEO architecture documentation, and focused SEO unit tests. Added `x-pathname` request propagation in middleware for path-aware root metadata.
2. **Decisions and source authority:** `bn-BD` remains the default unprefixed public URL; `en-BD` uses an explicit URL prefix; public pages emit reciprocal alternates and `x-default`; operational and sensitive surfaces are excluded from indexing. These decisions follow the milestone-specific requirements and existing canonical locale configuration.
3. **Behavior added:** Canonical URLs, OpenGraph, Twitter cards, locale alternates, Organization JSON-LD, BreadcrumbList JSON-LD, robots exclusions, and a public-only sitemap are generated server-side. Private metadata intentionally omits canonical/hreflang tags and emits `noindex,nofollow`.
4. **Tests and commands:** `bun run typecheck` passed; focused SEO tests passed: 4 tests, 0 failures; `bun run openapi` passed; `bun run build:local` was executed; `git diff --check` passed. Existing repository lint/test baseline exceptions remain documented from Milestone 058.
5. **Security, tenancy, localization, and financial checks:** Admin, seller, account, checkout, cart, order, wallet, authentication, verification, and API paths are excluded from crawling; locale URL generation uses canonical `en-BD`/`bn-BD`; no financial, wallet, reward, or tenant behavior was changed.
6. **Configuration still required:** Production must set `APP_URL`/`NEXT_PUBLIC_APP_URL` to the canonical public HTTPS origin. Product detail structured data remains intentionally limited because the current product page is client-rendered demo data rather than authoritative server-backed catalog data.
7. **Approved exceptions and next milestone:** No database migration or external provider was required. Product detail metadata currently uses the route slug until server-backed product metadata is available. Next milestone: **060 - Complete localization fallback, translation-admin, and QA tooling**.
