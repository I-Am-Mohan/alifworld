...
milestone: 268
phase: 27
phase_name: "Caching, Performance, Scale, and Resilience"
title: "Implement graceful degradation for search, storage, Redis, and queues"
status: not-started
currency: BDT
locales: [en-BD, bn-BD]
timezone: Asia/Dhaka
...
# Milestone 268: Implement graceful degradation for search, storage, Redis, and queues

## Outcome

Implement **Implement graceful degradation for search, storage, Redis, and queues** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 27, **Caching, Performance, Scale, and Resilience**. The phase purpose is: Prove that the stateless application can scale horizontally and degrade safely.
...
- Previous: [267 - Prove stateless horizontal scaling behind a load balancer](267-prove-stateless-horizontal-scaling-behind-a-load-balancer.md)
- Next: [269 - Execute performance, soak, and capacity tests](269-execute-performance-soak-and-capacity-tests.md)
...
- Business logic must depend on a SearchService interface, not Meilisearch directly.
- The app must boot and expose a documented degraded mode when search is unavailable.
- Treat Redis as external and unavailable at times; define fail-open or fail-closed per capability.
- Never make an irreversible ledger truth exist only in Redis.
- Web requests enqueue after the database transaction through an outbox-safe mechanism.
- Jobs need deterministic IDs, retry classification, backoff, and dead-letter visibility.
...
Primary expected areas for this phase: `lib/redis, lib/cache, lib/rate-limit, performance, operations`. The exact existing repository structure wins when it already enforces the same boundaries. Do not create duplicate parallel modules merely to match a suggested path.
...
Phase-level test emphasis: cache invalidation, lock expiry, load, soak, failover, backup and restore tests.
...
- [ ] The implementation of **Implement graceful degradation for search, storage, Redis, and queues** is real, reachable where intended, and contains no fake success path.
