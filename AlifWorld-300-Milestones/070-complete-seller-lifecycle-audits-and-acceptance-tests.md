...
title: "Complete seller lifecycle audits and acceptance tests"
status: completed
...
# Milestone 070: Complete seller lifecycle audits and acceptance tests

## Outcome

Implement **Complete seller lifecycle audits and acceptance tests** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 07, **Seller Lifecycle**. The phase purpose is: Onboard, approve, operate, and govern sellers and their staff safely.
...
- Previous: [069 - Build seller staff administration and activity visibility](069-build-seller-staff-administration-and-activity-visibility.md)
- Next: [071 - Implement hierarchical categories](071-implement-hierarchical-categories.md)
...
- [ ] The implementation of **Complete seller lifecycle audits and acceptance tests** is real, reachable where intended, and contains no fake success path.
...
- **Files changed:** Hardened centralized audit redaction for banking/KYC/private-object fields, added canonical seller lifecycle/staff/default audit actions, added deterministic seller lifecycle audit acceptance coverage, and added lifecycle audit architecture documentation. No Prisma migration was required.
- **Decisions:** Audit logs remain append-only and seller activity is filtered by seller-scoped staff resource metadata. Credential-like values and private object references are always redacted. Lifecycle transitions remain explicit and optimistic-version protected.
- **Acceptance coverage:** Tests cover lifecycle transition validity, seller-owner versus staff authorization, cross-tenant denial, banking/private-object redaction, and before/after diff redaction. Existing audit-service tests continue to cover request telemetry, query authorization, immutability, and non-blocking persistence failures.
- **Validation:** Focused lifecycle and audit tests, `bun run typecheck`, `bun run openapi`, `bun run build:local`, and `git diff --check` passed. The build reports existing repository lint warnings only.
- **Security and tenancy:** No credentials, tokens, banking data, KYC URLs, or private object keys are exposed in audit diffs or seller activity. Seller operations remain tenant-scoped and seller staff cannot administer staff by default. Financial, payout, commission, reward, and order calculations were unchanged.
- **Configuration:** No new environment variables or migrations are required.
- **Exceptions and next milestone:** Full database-backed lifecycle replay and multi-request concurrency acceptance require the isolated integration database pipeline. Next: **071 — Implement hierarchical categories**.
