...
title: "Localize validation, errors, notifications, and transactional content"
status: completed
...
# Milestone 058: Localize validation, errors, notifications, and transactional content

## Outcome

Implement **Localize validation, errors, notifications, and transactional content** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 06, **Bangladesh Localization**. The phase purpose is: Make Bangladesh the first operational market while keeping locale, currency, and geography extensible.
...
- Previous: [057 - Implement locale-aware date, number, currency, and text formatting](057-implement-locale-aware-date-number-currency-and-text-formatting.md)
- Next: [059 - Implement localized SEO metadata, hreflang, and canonical strategy](059-implement-localized-seo-metadata-hreflang-and-canonical-strategy.md)
...
- [ ] The implementation of **Localize validation, errors, notifications, and transactional content** is real, reachable where intended, and contains no fake success path.
...
1. **Files and migration changed:** Added localized API error serialization, bilingual error/notification/transactional catalogs, notification contracts and queue service, persisted `User.locale`, notification preference and delivery models, the reviewed Prisma migration `20260923030000_localize_notifications_and_transactional_content`, identity locale types/repository support, profile locale read/write support, and focused unit tests.
2. **Decisions and source authority:** Canonical `en-BD`/`bn-BD` locales, `bn-BD` default, stable machine error codes, mandatory delivery for `SECURITY` and `TRANSACTIONAL`, preference-gated `MARKETING`, and deterministic delivery idempotency follow this milestone's locked constraints and requirements.
3. **Behavior added:** Login, admin content, and customer profile validation/errors now use localized stable envelopes. Transactional templates interpolate locale-specific content. Customer locale is persisted through registration and profile updates. Notification delivery queues only enabled optional channels and de-duplicates by a deterministic SHA-256 event key.
4. **Tests and commands:** `bunx prisma format --schema prisma/schema.prisma` passed; `bunx prisma validate --schema prisma/schema.prisma` passed; `bun run typecheck` passed; focused unit tests passed: 18 tests, 0 failures; `git diff --check` passed.
5. **Security, tenancy, localization, and financial checks:** Profile updates remain authenticated and object-authorized; protected profile fields remain rejected; API errors avoid unknown internal messages; catalog parity and placeholder parity are tested; no monetary or Point behavior was changed; notification idempotency is enforced before provider delivery.
6. **Configuration still required:** Production email/SMS/push providers, credentials, worker processors, and provider-specific retry/dead-letter configuration remain disabled infrastructure boundaries for later notification milestones.
7. **Approved exceptions and next milestone:** Existing registration/resend outbox payloads still carry verification codes for the existing development/transactional flow and require a later protected-secret delivery boundary. No new financial approval gate was introduced. Next milestone: **059 - Implement localized SEO metadata, hreflang, and canonical strategy**.
