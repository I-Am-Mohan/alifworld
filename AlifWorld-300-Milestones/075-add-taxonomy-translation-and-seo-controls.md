...
title: "Add taxonomy translation and SEO controls"
status: completed
...
# Milestone 075: Add taxonomy translation and SEO controls

## Completion report (2026-09-23)

1. **Files and migration:** Added taxonomy translation SEO fields, effective-date `TaxRule` persistence and migration `20260923140000_taxonomy_seo_and_tax_rules`, locale-aware repository/service contracts, Admin translation and tax-rule routes, shared taxonomy SEO helper, OpenAPI paths/schemas, focused tests, and architecture documentation.
2. **Decisions:** Canonical taxonomy slugs remain stable; localized SEO metadata is stored per locale; English requests no longer fall back to Bengali; persisted active tax rules are resolved by jurisdiction, category specificity, and effective date while product/category overrides remain compatible.
3. **Behavior:** Added category/brand translation and SEO upserts, effective-date tax-rule CRUD, persisted tax-rule resolution, canonical/OpenGraph/Twitter/breadcrumb metadata helpers, and no-index behavior for private routes.
4. **Verification:** Prisma validate/generate, TypeScript typecheck, focused localization/SEO/tax tests, existing tax tests, OpenAPI generation, and `git diff --check` passed.
5. **Security and integrity:** Admin authorization and audit records protect taxonomy and tax-rule mutations; locale and date-range inputs are validated; tax calculations remain integer-poisha based; private route indexing remains disabled.
6. **Configuration:** No new credentials are required. Deploy migration `20260923140000_taxonomy_seo_and_tax_rules` before enabling persistence-backed tax rules.
7. **Approved exceptions and next milestone:** Existing category/brand/product public pages still need direct data-backed SEO resolver wiring as catalog pages mature. Tax snapshot storage on future transactional records remains deferred. Next milestone: 076, catalog permissions and product approval workflow.

## Outcome

Implement **Add taxonomy translation and SEO controls** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 08, **Catalog Foundations**. The phase purpose is: Build governed taxonomy and catalog primitives usable by sellers, customers, Admin, and search.
...
- Previous: [074 - Implement attributes, values, and variant option sets](074-implement-attributes-values-and-variant-option-sets.md)
- Next: [076 - Implement catalog permissions and product approval workflow](076-implement-catalog-permissions-and-product-approval-workflow.md)
...
- Generate canonical, OpenGraph, Twitter, breadcrumb, and appropriate schema.org data from server data.
- Exclude Admin, Seller, account, checkout, and sensitive routes from indexing.
...
- [ ] The implementation of **Add taxonomy translation and SEO controls** is real, reachable where intended, and contains no fake success path.
