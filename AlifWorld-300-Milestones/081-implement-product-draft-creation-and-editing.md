...
title: "Implement product draft creation and editing"
status: completed
...
# Milestone 081: Implement product draft creation and editing

## Completion report (2026-09-23)

1. **Files and APIs:** Added seller-scoped `GET/PATCH /api/v1/seller/catalog/products/{id}` draft endpoints, strengthened ProductService category/brand/currency checks, added an API-backed Seller draft editor, OpenAPI schemas, focused tests, and architecture documentation.
2. **Decisions:** Sellers create only `DRAFT` products and may edit only `DRAFT` or `REJECTED` products; approval status remains controlled by the Milestone 076 workflow; seller scope is derived from the authenticated actor; slug changes preserve redirect history.
3. **Behavior:** The editor supports localized titles, descriptions, canonical slug, category/brand references, BDT poisha pricing, independent Product Points, SKU/barcode, warranty, and tags. Updates use optimistic versions and reject status changes from Seller routes.
4. **Verification:** TypeScript typecheck, product draft/editing tests, existing ProductService tests, OpenAPI generation, and `git diff --check` passed.
5. **Security and integrity:** Seller tenant ownership is checked before reads and updates; inactive/unapproved taxonomy references are rejected; seller status changes and publication are blocked; slug history and audit/outbox behavior are preserved.
6. **Configuration:** No new credentials are required. Existing catalog migrations must be deployed before using the editor.
7. **Approved exceptions and next milestone:** Variant matrix generation, complete media upload orchestration, and full category-driven field selection remain subsequent workflow work. Next milestone: 082, variant combination generation and validation.

## Outcome

Implement **Implement product draft creation and editing** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 09, **Products and Media**. The phase purpose is: Deliver production product and variant management with mandatory BDT price and Product Point values.
...
- Previous: [080 - Complete catalog APIs, Admin UI, Seller UI, and tests](080-complete-catalog-apis-admin-ui-seller-ui-and-tests.md)
- Next: [082 - Implement variant combination generation and validation](082-implement-variant-combination-generation-and-validation.md)
...
- [ ] The implementation of **Implement product draft creation and editing** is real, reachable where intended, and contains no fake success path.
