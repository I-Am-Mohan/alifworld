...
title: "Implement safe bulk catalog import and export"
status: completed
...
# Milestone 078: Implement safe bulk catalog import and export

## Completion report (2026-09-23)

1. **Files and migration:** Added `CatalogImportJob`, `CatalogImportRowError`, and `CatalogExportJob` with migration `20260923170000_catalog_import_export`; bulk contracts, repository, service, seller APIs, Seller bulk UI, worker queue registration, OpenAPI schemas, focused tests, and architecture documentation.
2. **Decisions:** Imports support bounded CSV/JSON payloads, private object storage, dry-run validation, explicit commit, seller-scoped draft creation, row errors, and idempotency keys. Exports are CSV-only with private storage and short-lived signed URLs.
3. **Behavior:** Added validation for required fields, active categories, approved brands, BDT integer poisha, Product Points, duplicate slugs, row limits, and payload size. Commit is transactional and emits audit/outbox records. Export filters remain seller-scoped.
4. **Verification:** Prisma validate/generate, TypeScript typecheck, focused bulk/onboarding/approval tests, OpenAPI generation, and `git diff --check` passed. Production build remains to be run in the final validation pass.
5. **Security and integrity:** Seller ownership/staff checks are applied inside services and direct seller-scoped queries; object keys are never exposed; signed URLs expire; dry-run does not mutate products; commit creates only seller-owned drafts.
6. **Configuration:** No new credentials are required beyond the existing S3-compatible storage configuration. Deploy migration `20260923170000_catalog_import_export` before enabling persistence.
7. **Approved exceptions and next milestone:** Bounded import/export processing currently runs synchronously; the worker queue contract is registered for larger asynchronous jobs in later hardening. Next milestone: 079, catalog moderation and duplicate detection.

## Outcome

Implement **Implement safe bulk catalog import and export** as a production-ready, tested part of AlifWorld. This milestone belongs to Phase 08, **Catalog Foundations**. The phase purpose is: Build governed taxonomy and catalog primitives usable by sellers, customers, Admin, and search.
...
- Previous: [077 - Create seller catalog onboarding templates and guidance](077-create-seller-catalog-onboarding-templates-and-guidance.md)
- Next: [079 - Implement catalog moderation and duplicate detection](079-implement-catalog-moderation-and-duplicate-detection.md)
...
- [ ] The implementation of **Implement safe bulk catalog import and export** is real, reachable where intended, and contains no fake success path.
